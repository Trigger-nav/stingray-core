...
| 0.5 | GRIB pipeline in production: NOMADS + ECMWF open data, cfgrib parsing | `ingest/fetch_grib_nomads.py` + `fetch_grib_ecmwf.py` written and unit-tested (normalisation logic + real committed GRIB2 fixtures), but **not yet run end-to-end** — no `eccodes` in the scoping sandbox to actually exercise `cfgrib`. See CLAUDE.md's GRIB-conventions gotcha for the "first real run" checklist before trusting this in production. Scheduling deliberately out of scope (edge-first — belongs with the Phase 1+ edge device, not this bench milestone). |
