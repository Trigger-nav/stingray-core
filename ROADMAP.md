...
| 0.6 | Twin v1 offline: fit/validate tooling for the parametric components | **Approach (agreed July 2026): build now against synthetic data — do not wait for the naval-arch consult.** Scope: (a) fitting pipeline for the twin's components (calm-water curve, added resistance, SFOC maps) from telemetry-shaped input; (b) steady-state segment extraction (reject manoeuvring/transients/tank-transfer artefacts); (c) priors as per-parameter distributions in `VesselSpec`, seeded from published empirical methods (Holtrop–Mennen resistance, standard SFOC map shapes, STAWAVE-class added resistance), every prior **marked provisional pending naval-arch review**; (d) fit quality reported as error bands / holdout validation, not point estimates. **Acceptance test (the core one, needs no real data): generate synthetic telemetry from a twin with known parameters + realistic noise + injected junk segments → pipeline recovers the known parameters within stated tolerance and rejects the junk.** Consultant engagement is a later, short review of 0.6's model forms + 0.7's draft together, before the Phase 1 pilot fit (ticket 1.5). |
