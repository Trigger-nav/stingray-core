...
- **UI:** one glanceable bridge screen, centred on a **vessel-state visual** — a top-down representation of the yacht surrounded by live per-parameter optimality (speed setpoint, engine config/load, sea-state exposure, added resistance), with relative wave/wind indicators. The core controls are two per-passage sliders — **Pace** (Economy ↔ Schedule) and **Comfort** (Crew transit ↔ Owner aboard) — the only objectives that genuinely change passage to passage. **Machinery care is vessel policy, not a passage input**: set once by the chief engineer (max continuous load, slamming-avoidance threshold, load-cycling limits), applied under every plan, with wear surfaced as an output metric on each plan card. Presets ("Owner aboard", "Repositioning", "Charter turnaround", "Delivery") remain the primary interface and encode all weightings.
...
- **Two-page structure:** *Plan* and *Underway*. Planning is a spatial decision, so the chart leads — candidate routes and weather field centre-stage, with a prominent "Begin Passage" action once a plan is selected. Underway is an execution task, so the vessel-state display leads — Course to Steer, target BSP, RPM, engine config, and next-alteration setpoints, each shown **target vs live** (bus-fed actuals with colour-coded deviation) — with the chart as a compact progress overview.
- **Chart posture:** Stingray does not replace navigation software. Its chart supports plan comparison and progress monitoring only; approved plans export to ECDIS (RTZ), where navigation properly lives.
