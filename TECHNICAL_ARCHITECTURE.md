...
- **Decision record — grid vs isochrone (July 2026):** isochrone methods were evaluated and rejected as the core: their frontier-dominance pruning assumes single-objective time-optimality and open water, both false for Stingray (blended objectives, variable speed, confined Med waters fragment the frontier). Isochrones are retained in two supporting roles: (1) a cheap time-only reachability sweep as a lattice pre-pruning pass (bounds the corridor within the ETA window before A* runs); (2) a regression cross-check — with pure-schedule weights, the grid route must converge to the time-optimal isochrone route.
